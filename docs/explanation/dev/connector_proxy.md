...
A connector proxy can make outbound requests to external systems, so security controls are critical.

### 1) Prevent SSRF and unsafe outbound access
- Enforce an allowlist of approved domains/hosts
- Block access to internal metadata endpoints and private ranges where applicable
- Validate and normalize URLs before requesting them

### 2) Keep credentials out of BPMN and repositories
- Store secrets in environment variables or a secret manager
- Avoid committing tokens/passwords to source control

### 3) Require authentication/authorization
The example enables CORS and does not include authentication. In production:
- place the proxy behind an API gateway or service-to-service auth
- restrict which callers can execute commands

### 4) Add timeouts, limits, and rate controls
- configure connect/read timeouts
- limit payload sizes and response sizes where possible
- add rate limiting to prevent abuse

### 5) Logging and redaction
- log request IDs and outcomes
- redact sensitive fields (tokens, passwords, Authorization headers)
- avoid logging full payloads unless required and properly filtered

### 6) Environment separation
- use staging/mock services for workflow development and testing
- avoid pointing development proxies directly at production services