...
If the callback reports a transient failure and the service task has retries configured, SpiffArena keeps the service task waiting, applies the configured retry backoff, and calls the connector again for the next attempt.
Without retry configuration, or after retries are exhausted, the callback failure moves the process to error.
...
5. The process resumes and continues to the next task. If the callback reports a transient failure, configured service task retries are handled by SpiffArena rather than the connector proxy.
