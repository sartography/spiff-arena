...
This will create a zip file containing the [Lambda entry point function](https://github.com/sartography/connector-proxy-lambda-demo/blob/main/connector_proxy_lambda_demo/lambda_function.py) and all the dependencies needed to execute the connectors.
...
Opt for `Author from scratch` and select a supported Python 3 runtime.
...
## Integrating With Spiff Arena

Congratulations, your Connector Proxy has been deployed as a Lambda function.
For information on configuring Spiff Arena to use the new Connector Proxy URL, please see [Configure a Connector Proxy](configure_a_connector_proxy).
