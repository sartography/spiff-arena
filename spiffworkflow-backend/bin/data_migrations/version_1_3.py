...
class VersionOneThree:
    ...
    def get_relevant_task_definitions(self) -> list[TaskDefinitionModel]:
        task_definitions: list[TaskDefinitionModel] = TaskDefinitionModel.query.filter_by(typename='_BoundaryEventParent').all()
    ...
    def process_task_definition(self, task_definition: TaskDefinitionModel) -> None:
        ...
        for parent_bpmn_identifier in properties_json['inputs']:
            parent_task_definition = TaskDefinitionModel.query.filter_by(bpmn_identifier=parent_bpmn_identifier, bpmn_process_definition_id=task_definition.bpmn_process_definition_id).first()
        ...
        for child_bpmn_identifier in properties_json['outputs']:
            child_task_definition = TaskDefinitionModel.query.filter_by(bpmn_identifier=child_bpmn_identifier, bpmn_process_definition_id=task_definition.bpmn_process_definition_id).first()
