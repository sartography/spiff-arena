...
def queue_future_task_if_appropriate(process_instance: ProcessInstanceModel, eta_in_seconds: float, task_guid: str) -> bool:
    if queue_enabled_for_process_model(process_instance):
        ...
        celery.current_app.send_task('spiffworkflow_backend.background_processing.celery_tasks.process_instance_task.celery_task_process_instance_run', kwargs=args_to_celery, countdown=countdown + 1)  # type: ignore
        return True
...
def queue_process_instance_if_appropriate(process_instance: ProcessInstanceModel) -> bool:
    if queue_enabled_for_process_model(process_instance):
        celery.current_app.send_task('spiffworkflow_backend.background_processing.celery_tasks.process_instance_task.celery_task_process_instance_run', (process_instance.id,))  # type: ignore
        return True
