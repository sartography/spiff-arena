...
def queue_process_instance_if_appropriate(process_instance: ProcessInstanceModel, eta: datetime | None = None) -> bool:
    if queue_enabled_for_process_model(process_instance) and process_instance.is_immediately_runnable():
        process_instance_task_run.delay(process_instance.id, eta=eta)  # type: ignore
        return True
