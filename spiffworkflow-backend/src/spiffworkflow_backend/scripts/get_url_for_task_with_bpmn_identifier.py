...
class GetUrlForTaskWithBpmnIdentifier(Script):
    ...
    def get_description(self) -> str:
        return """Returns the url to the task show page for a task with the given bpmn identifier. The script task calling this MUST be in the same process as the desired task and should be next to each other in the diagram."""
    ...
    def run(self, script_attributes_context: ScriptAttributesContext, *args: Any, **kwargs: Any) -> Any:
        ...
        if desired_spiff_task is None:
            raise Exception(f"Could not find a task with bpmn identifier '{bpmn_identifier}' in get_url_for_task_with_bpmn_identifier")

        if not desired_spiff_task.task_spec.manual:
            raise Exception(f"Given bpmn identifier ({bpmn_identifier}) represents a task that cannot be completed by people and therefore it does not have a url to retrieve")
