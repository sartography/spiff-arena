...
class BpmnProcessService:
    ...
    def _lookup_bpmn_process_definition_after_insert_or_ignore(
        ...
        for attempt in range(cls.BPMN_PROCESS_DEFINITION_LOOKUP_MAX_ATTEMPTS):
            ...
            # If another request won the insert but has not committed yet, start the
            # next lookup in a fresh transaction so databases with transaction snapshots
            # can see the row once it becomes visible.
            db.session.rollback()
