...
class SpecFileService(FileSystemService):
    ...
    @classmethod
    def extract_human_task_definitions(
        cls, process_model_info: ProcessModelInfo, files: list[File], visited_processes: set | None = None
    ) -> list[dict[str, Any]]:
        if visited_processes is None:
            visited_processes = set()

        if process_model_info.id in visited_processes:
            return []
        visited_processes.add(process_model_info.id)
        ...
        for file in bpmn_files:
            ...
            # Extract user tasks from the current file
            for task in bpmn_etree.xpath("//bpmn:userTask | //bpmn:manualTask", namespaces=ns):
                name = task.get("name")
                task_id = task.get("id")
                local_name = etree.QName(task).localname
                task_type = local_name[0].upper() + local_name[1:]
                task_info = {
                    "name": name if name else task_id,
                    "bpmn_identifier": task_id,
                    "bpmn_name": name,
                    "task_type": task_type,
                    "file": file.name,
                }
                human_tasks.append(task_info)

            # Handle call activities
            for call_activity in bpmn_etree.xpath("//bpmn:callActivity", namespaces=ns):
                called_element = call_activity.get("calledElement")
                if called_element:
                    # Find the process model for the called element
                    ref_cache = ReferenceCacheModel.query.filter_by(identifier=called_element, type="process").first()
                    if ref_cache:
                        sub_process_model_info = ProcessModelService.get_process_model(ref_cache.relative_location)
                        if sub_process_model_info:
                            sub_process_files = ProcessModelService.get_process_model_files(sub_process_model_info)
                            # Recursive call
                            human_tasks.extend(
                                cls.extract_human_task_definitions(sub_process_model_info, sub_process_files, visited_processes)
                            )
