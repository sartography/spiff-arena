...
def test_delete_process_instances_with_criteria_writes_run_log(monkeypatch: pytest.MonkeyPatch) -> None:
    class FakeStore:
        id = 10
    ...
    class FakeStoreQuery:
        def filter_by(self, **kwargs: Any) -> "FakeStoreQuery":
            return self

        def first(self) -> FakeStore:
            return FakeStore()

    class FakeEntryQuery:
        def filter_by(self, **kwargs: Any) -> "FakeEntryQuery":
            return self

        def first(self) -> None:
            return None

    class FakeLogSession(FakeSession):
        def query(self, model: Any) -> FakeStoreQuery | FakeEntryQuery:
            if model is FakeStore:
                return FakeStoreQuery()
            return FakeEntryQuery()

    results = [SimpleNamespace(id=1, process_model_identifier="cleanup/model-a", status="complete")]
    ...
    monkeypatch.setattr(script_module, "KKVDataStoreModel", FakeStore)
    monkeypatch.setattr(script_module, "KKVDataStoreEntryModel", FakeEntry)
    monkeypatch.setattr(script_module, "db", SimpleNamespace(session=session))
    monkeypatch.setattr(script_module, "or_", lambda *args: args)

    DeleteProcessInstancesWithCriteria().run(
        ScriptAttributesContext(
            task=None,
            environment_identifier="unit_testing",
        ...
        [{"status": ["complete"], "last_updated_delta": 60}],
        limit=1,
        return_summary=True,
        run_log_data_store_identifier="reaper_run_log",
        run_log_key="process-instance-22",
        run_log_secondary_key="batch-0",
    )

    assert session.added[0].top_level_key == "process-instance-22"
    assert session.added[0].secondary_key == "batch-0"
    assert session.added[0].value["total_deleted"] == 1
