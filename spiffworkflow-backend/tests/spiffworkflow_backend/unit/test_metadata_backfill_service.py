...
class TestMetadataBackfillService(BaseTest):
    def test_detect_metadata_changes(
        ...
        old_model = ProcessModelInfo(
            ...
            metadata_extraction_paths=[
                {"key": "existing_key", "path": "existing.path"}
            ]
        )
    ...
    def test_extract_metadata_for_instance(
        ...
        # Test
        result = MetadataBackfillService.extract_metadata_for_instance(
            "test_model", task_data, metadata_paths
        )
    ...
    def test_backfill_metadata_for_model(
        ...
        with patch.object(MetadataBackfillService, 'get_process_instance_count', return_value=3):
            ...
            with patch.object(MetadataBackfillService, 'get_process_instances', return_value=mock_instances), \
                ...
                # Run the test
                result = MetadataBackfillService.backfill_metadata_for_model(
                    process_model.id, new_metadata_paths
                )
