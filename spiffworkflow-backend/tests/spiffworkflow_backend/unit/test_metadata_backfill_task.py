...
from spiffworkflow_backend.background_processing.celery_tasks.metadata_backfill_task import _celery_task_backfill_metadata_impl, celery_task_backfill_metadata
...
class TestMetadataBackfillTask(BaseTest):
    ...
    def test_celery_task_backfill_metadata(
        ...
        mock_metadata_backfill_service.backfill_metadata_for_model.assert_called_once_with(
            "test_process_model", metadata_paths
        )
    ...
    def test_trigger_metadata_backfill(
        ...
        new_model = ProcessModelInfo(
            ...
            metadata_extraction_paths=[
                {"key": "existing", "path": "existing.path"},
    ...
    def test_trigger_metadata_backfill_disabled(
        ...
        mock_current_app.config = {
            "SPIFFWORKFLOW_BACKEND_PROCESS_INSTANCE_METADATA_BACKFILL_ENABLED": False
        }
