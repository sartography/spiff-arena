...
class TestNotificationTrigger(BaseTest):
    @patch("spiffworkflow_backend.services.process_instance_processor.ProcessInstanceProcessor.setup_processor_with_process_instance")
    ...
    def test_trigger_notification_process_model_with_user(
        ...
        # Execute
        processor._trigger_notification_process_model(
            notification_pm_id, human_task, potential_owner_hash
        )
        ...
        mock_create_and_run.assert_called_once_with(
            mock_notification_pm, persistence_level="full", data_to_inject=expected_data
        )
    ...
    @patch("spiffworkflow_backend.services.process_instance_processor.ProcessInstanceProcessor.setup_processor_with_process_instance")
    @patch("spiffworkflow_backend.services.process_instance_processor.ProcessInstanceProcessor._trigger_notification_process_model")
    ...
    @patch("spiffworkflow_backend.services.process_instance_processor.ProcessInstanceProcessor._extract_task_metadata_from_extensions")
    @patch("spiffworkflow_backend.services.process_instance_processor.ProcessInstanceProcessor._extract_form_properties_from_extensions")
    ...
    def test_process_ready_or_waiting_task_triggers_notification(
        ...
        ready_task.task_spec.extensions = {
            "properties": {
                "notificationProcessModel": "my_notification_model"
            }
        }
        ...
        # Verify
        mock_trigger_notification.assert_called_once_with(
            "my_notification_model", human_task, potential_owners
        )
