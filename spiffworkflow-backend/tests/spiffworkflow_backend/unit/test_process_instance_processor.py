...
class TestProcessInstanceProcessor(BaseTest):
    ...
    def test_large_multiinstance(
        ...
        process_model = load_test_spec(
            process_model_id="test_group/multiinstance_with_data",
            process_model_source_directory="multiinstance_with_data",
        )
        ...
        print(f"➡️ ➡️ ➡️  Execution duration: {duration}")

        start_time = time.time()
        processor.save()
        end_time = time.time()
        duration = end_time - start_time
        print(f"➡️ ➡️ ➡️  Serialize duration: {duration}")

    # To test processing times with multiinstance subprocesses
    def test_large_multiinstance_with_some_data(
        self,
        app: Flask,
        client: TestClient,
        with_db_and_bpmn_file_cleanup: None,
        ...
        start_time = time.time()
        reloaded_processor = ProcessInstanceProcessor(ProcessInstanceModel.query.get(process_instance_id))
        end_time = time.time()
        duration = end_time - start_time
        print(f"➡️ ➡️ ➡️  Deserialize duration: {duration}")

        # Verify the deserialized processor is functional
        assert reloaded_processor.process_instance_model is not None
        assert reloaded_processor.bpmn_process_instance is not None
