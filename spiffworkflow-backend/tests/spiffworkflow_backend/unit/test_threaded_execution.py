...
class TestThreadedExecution(BaseTest):
    ...
    def assert_same_start_times(self, process_instance, task_name_starts_with, expected_size):
        ...
        script_tasks = list(filter(lambda tm: tm.task_definition.bpmn_name is not None and
                                              tm.task_definition.bpmn_name.startswith(task_name_starts_with),
                                   task_models))
