...
def test_process_model_import(browser_context: BrowserContext):
    ...
    # Wait for validation to complete (look for the checkmark or preview data)
    # We might need to wait a bit for the backend to validate the URL
    page.wait_for_timeout(2000)  # 2 second timeout
    
    # No custom ID input - backend will automatically handle ID generation
    
    # Click the import button
    import_model_button = page.get_by_test_id("import-button")
    expect(import_model_button).to_be_enabled()
    import_model_button.click()
    ...
    # For the example repository, this would be "0-1-minimal-example" or "0-1-minimal-example-{timestamp}"
    ...
    # Find the imported process model and delete it
    # This part depends on the UI for deleting process models
    # We're assuming there's a delete button or menu option
    
    # Example cleanup using command line (will be run after the test)
    # The actual test will rely on the ./bin/clean_up_sample_process_models script
